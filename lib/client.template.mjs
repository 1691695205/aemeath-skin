...
function apply(_ctx) {
	...
	const mountDecorations = () => {
		...
		// ---- academy emblem seal (anchored to conversation top-left) ----
		const emblem = doc.createElement("img");
		emblem.src = asset("emblem.png");
		emblem.dataset.skinOwner = "aemeath";
		emblem.setAttribute("aria-hidden", "true");
		emblem.style.cssText =
			"position:fixed;left:16px;top:80px;width:132px;height:auto;opacity:0.55;" +
			"mix-blend-mode:screen;pointer-events:none;z-index:45;user-select:none;-webkit-user-drag:none;" +
			"transition:opacity .4s,left .2s ease,top .2s ease";
		// Track the center conversation column; keep the seal pinned to its
		// top-left corner (fixed to the viewport, not to the scrolling rows).
		const CONV_SEL = "[data-pane='conversation'], [class*='_workbench']";
		const syncEmblem = () => {
			const conv = doc.querySelector(CONV_SEL);
			const r = conv && conv.getBoundingClientRect().width > 0 ? conv.getBoundingClientRect() : null;
			if (r && r.width > 0) {
				emblem.style.left = Math.max(6, Math.round(r.left + 12)) + "px";
				emblem.style.top = Math.max(6, Math.round(r.top + 88)) + "px";
				emblem.style.bottom = "auto";
				emblem.style.opacity = "0.55";
			} else {
				// no conversation mounted yet — hide until it appears
				emblem.style.opacity = "0";
			}
		};
		syncEmblem();
		if (typeof ResizeObserver !== "undefined") {
			const emblemRO = new ResizeObserver(syncEmblem);
			emblemRO.observe(doc.body);
			onCleanup(() => emblemRO.disconnect());
		}
		doc.addEventListener("scroll", syncEmblem, true);
		doc.defaultView?.addEventListener("resize", syncEmblem);
		onCleanup(() => {
			doc.removeEventListener("scroll", syncEmblem, true);
			doc.defaultView?.removeEventListener("resize", syncEmblem);
			emblem.remove();
		});
		body.append(emblem);
		...
		// ---- background art (theme-switched, fixed layer) ----
		// Uses a dedicated position:fixed layer instead of body background +
		// background-attachment:fixed — the latter is unreliable in newer
		// Chromium (Electron 43 / DSH 2.0.3) with large data-URI images and
		// can silently not paint. A fixed child div paints on every version.
		...
		const bgLayer = doc.createElement("div");
		bgLayer.dataset.aemeathBg = "";
		...
		const applyBg = () => {
			...
			const bg = scrim + ", url('" + asset(dark ? "palace-dark.jpg" : "palace-light.jpg") + "')";
			bgLayer.style.backgroundImage = bg;
			// keep the body fallback too (identical, painted below the layer)
